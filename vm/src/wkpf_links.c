...
uint16_t wkpf_get_component_id(uint8_t port_number) {
  ...
  return 0; // Should never happen since there should always be a component at this port
}
...
uint8_t wkpf_propagate_property(uint8_t port_number, uint8_t property_number, int16_t value) {
  uint16_t component_id = wkpf_get_component_id(port_number);
  ...
  for(int i=0; i<number_of_links; i++) {
    if(links[i].src_component_id == component_id
      ...
      if (dest_node_id == nvmcomm_get_node_id()) {
        ...
        wkpf_error_code = wkpf_get_endpoint_by_port(port_number, &dest_endpoint);
