...
def reprogramNvmdefault(destination, filename):
  MESSAGESIZE = 16
  pynvc.sendcmd(destination, pynvc.REPRG_OPEN)
  reply = checkedReceive(1000, [pynvc.REPRG_OPEN_R])
  ...
  while not pos == len(bytecode):
    ...
    if pos/pagesize == (pos+len(payload_data))/pagesize:
      ...
      reply = checkedReceive(1000,[pynvc.REPRG_WRITE_R_OK, pynvc.REPRG_WRITE_R_RETRANSMIT])
    ...
    if pos == len(bytecode):
      ...
      pynvc.sendcmd(destination, pynvc.REPRG_COMMIT, payload)
      reply = checkedReceive(1000, [pynvc.REPRG_COMMIT_R_RETRANSMIT,
