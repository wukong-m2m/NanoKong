...
Block.prototype.draw=function() {
	...
	var size = this.getSize();
	this.div.empty();
    this.div.append('<span style="font-size: 100%; word-wrap: break-word;">' + this.type.replace('_', ' ') + '</span>');
	for(i=0;i<this.signals.length;i++) {
		...
		$('#signal_'+this.id+'_'+i).html('<span style="font-size: 200%; word-wrap: break-word;">' + this.signals[i].name.replace('_', ' ') + '</span>');
	}
	for(i=0;i<this.actions.length;i++) {
		...
		$('#action_'+this.id+'_'+i).html('<span style="font-size: 200%; word-wrap: break-word;">' + this.actions[i].name.replace('_', ' ') + '</span>');
	}
}
